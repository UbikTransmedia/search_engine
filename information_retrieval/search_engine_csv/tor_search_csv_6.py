...
class LinkRelationshipManager:
    ...
    def add_relationship(self, from_url, to_url):
        try:
            self.link_relationships[from_url].add(to_url)
            self._save_json()
            print(f"{Fore.BLUE}🔗 Relationship added: {from_url} -> {to_url}")
        except Exception as e:
            print(f"{Fore.RED}❌ Failed to add relationship: {e}")
        ...
        try:
            ...
            for a_tag in soup.find_all('a', href=True):
                ...
                if link.startswith('http'):
                    ...
                    self.relationship_manager.add_relationship(url, link)  # Track relationships
                    links.add(link)
    ...
    def run(self):
        while True:
            try:
                ...
                print(f"{Fore.RED}❌ ReverseIndexer encountered an error: {e}")
                time.sleep(5)  # Wait before restarting
...
class VisualizationManager(threading.Thread):
    ...
    def create_term_cloud(self):
        try:
            ...
            print(f"{Fore.GREEN}☁️ Term cloud created as term_cloud.png")
        except Exception as e:
            print(f"{Fore.RED}❌ Failed to create term cloud: {e}")
    ...
    def create_link_cloud(self):
        try:
            ...
            nx.draw(G, with_labels=False, node_size=20, edge_color="blue", node_color="red")
            plt.savefig("link_cloud.png")
            print(f"{Fore.GREEN}🔗 Link cloud created as link_cloud.png")
        except Exception as e:
            print(f"{Fore.RED}❌ Failed to create link cloud: {e}")
    ...
    def run(self):
        while True:
            try:
                ...
                print(f"{Fore.RED}❌ VisualizationManager encountered an error: {e}")
                time.sleep(5)  # Wait before restarting
