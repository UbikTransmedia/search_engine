...
class LinkRelationshipManager:
    ...
    def add_relationship(self, from_url, to_urls):
        try:
            ...
            current_relationships = list(self.link_relationships.get(from_url, []))
...
class KPIManager(threading.Thread):
    def __init__(self, link_index_manager, reverse_index_manager, relationship_manager, crawled_sites, kpi_file='kpis.json'):
        threading.Thread.__init__(self)
        self.link_index_manager = link_index_manager
        self.reverse_index_manager = reverse_index_manager
        self.relationship_manager = relationship_manager
        self.crawled_sites = crawled_sites
    ...
    def calculate_kpis(self):
        try:
            # Copy all the data into independent variables
            links_copy = deepcopy(self.link_index_manager.get_all_links())
            reverse_index_copy = deepcopy(self.reverse_index_manager.get_reverse_index())
            relationships_copy = deepcopy(self.relationship_manager.link_relationships)
            crawled_sites_copy = deepcopy(self.crawled_sites)

            total_links = len(links_copy)
            total_terms = len(reverse_index_copy)
            total_relationships = sum(len(urls) for urls in relationships_copy.values())
            mean_response_time = self._calculate_mean_response_time(crawled_sites_copy)
            mutual_link_ratio = self._calculate_mutual_link_ratio(relationships_copy)
            link_loops_count = self._count_link_loops(relationships_copy)
    ...
    def _calculate_mean_response_time(self, crawled_sites):
        try:
            response_times = [site["response_time"] for site in crawled_sites.values() if "response_time" in site]
            if response_times:
    ...
    def _calculate_mutual_link_ratio(self, relationships):
        try:
            mutual_links = 0
            total_links = 0

            for from_url, to_urls in relationships.items():
                for to_url in to_urls:
                    total_links += 1
                    if to_url in relationships and from_url in relationships[to_url]:
                        mutual_links += 1
    ...
    def _count_link_loops(self, relationships):
        try:
            link_loops = 0
            for from_url, to_urls in relationships.items():
...
if __name__ == "__main__":
    ...
    kpi_manager = KPIManager(link_index_manager, reverse_index_manager, relationship_manager, crawler.crawled_sites)
