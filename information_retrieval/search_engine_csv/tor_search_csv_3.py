import threading
import requests
from bs4 import BeautifulSoup
import re
import time
import json
import os
from collections import defaultdict
from datetime import datetime, timedelta
from stem import Signal
from stem.control import Controller
from nltk.corpus import stopwords
import math
import logging
import ssl

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

class TorSession:
    def __init__(self, tor_password=None, port=9051, proxy_port=9050):
        self.tor_password = tor_password
        self.port = port
        self.proxy_port = proxy_port
        self.session = None
        self.ssl_context = ssl.create_default_context()

    def connect(self):
        attempt = 0
        while self.session is None and attempt < 5:  # Retry connection up to 5 times
            try:
                with Controller.from_port(port=self.port) as controller:
                    if self.tor_password:
                        controller.authenticate(password=self.tor_password)
                    else:
                        controller.authenticate()
                    controller.signal(Signal.NEWNYM)
                    logging.info("Connected to TOR with SSL support")
                self.session = requests.Session()
                self.session.proxies = {
                    'http': f'socks5h://127.0.0.1:{self.proxy_port}',
                    'https': f'socks5h://127.0.0.1:{self.proxy_port}'
                }
            except Exception as e:
                logging.error(f"Failed to connect to TOR: {e}")
                attempt += 1
                time.sleep(5)  # Wait before retrying

    def get_session(self):
        if not self.session:
            self.connect()
        return self.session

class JSONManager:
    def __init__(self, index_file='reverse_index.json'):
        self.index_file = index_file
        self.data = {
            "links": [],
            "reverse_index": {}
        }

        if os.path.exists(self.index_file):
            with open(self.index_file, 'r') as file:
                self.data = json.load(file)
        else:
            self._save_json()

    def _save_json(self):
        with open(self.index_file, 'w') as file:
            json.dump(self.data, file, indent=4)

    def insert_link(self, url, content, title=None, date=None, metadata=None):
        try:
            existing_id = self.get_link_id(url)
            if existing_id:
                return existing_id

            next_id = len(self.data["links"]) + 1
            link_info = {
                "id": next_id,
                "url": url,
                "content": content,
                "title": title,
                "date": date,
                "metadata": metadata,
                "terms": defaultdict(list)
            }

            tokens = content.split()
            for pos, token in enumerate(tokens):
                link_info["terms"][token].append(pos)

            self.data["links"].append(link_info)
            self.update_reverse_index(link_info, next_id)
            self._save_json()
            return next_id
        except Exception as e:
            logging.error(f"Failed to insert link: {e}")
            return None

    def update_reverse_index(self, link_info, doc_id):
        try:
            for term, positions in link_info["terms"].items():
                if term not in self.data["reverse_index"]:
                    self.data["reverse_index"][term] = {}
                self.data["reverse_index"][term][doc_id] = positions
        except Exception as e:
            logging.error(f"Failed to update reverse index: {e}")

    def get_link_id(self, url):
        try:
            for link in self.data["links"]:
                if link["url"] == url:
                    return link["id"]
            return None
        except Exception as e:
            logging.error(f"Failed to get link ID: {e}")
            return None

    def get_all_links(self):
        try:
            return self.data["links"]
        except Exception as e:
            logging.error(f"Failed to fetch links: {e}")
            return []

    def get_reverse_index(self):
        try:
            return self.data["reverse_index"]
        except Exception as e:
            logging.error(f"Failed to fetch reverse index: {e}")
            return {}

class LinkCrawler(threading.Thread):
    def __init__(self, tor_session, json_manager, seed_urls, depth=100, retries=5):
        threading.Thread.__init__(self)
        self.session = tor_session.get_session()
        self.json_manager = json_manager
        self.seed_urls = seed_urls
        self.depth = depth
        self.retries = retries

    def sanitize_content(self, text):
        try:
            text = text.lower()
            text = re.sub(r'\W+', ' ', text)
            tokens = text.split()
            stop_words = set(stopwords.words('english'))
            sanitized_tokens = [word for word in tokens if word not in stop_words]
            return ' '.join(sanitized_tokens)
        except Exception as e:
            logging.error(f"Failed to sanitize content: {e}")
            return ""

    def extract_metadata(self, soup):
        try:
            title = soup.title.string if soup.title else None
            date = soup.find('meta', {'name': 'date'}) or soup.find('meta', {'property': 'og:date'})
            date = date['content'] if date else None
            metadata = {meta.attrs['name']: meta.attrs['content'] for meta in soup.find_all('meta') if 'name' in meta.attrs and 'content' in meta.attrs}
            return title, date, metadata
        except Exception as e:
            logging.error(f"Failed to extract metadata: {e}")
            return None, None, None

    def crawl(self, url, retries_left=None):
        if not self.session:
            logging.error("No TOR session available")
            return set()

        if retries_left is None:
            retries_left = self.retries

        try:
            response = self.session.get(url, timeout=120)
            soup = BeautifulSoup(response.content, 'html.parser')
            text = soup.get_text(separator=' ')
            sanitized_text = self.sanitize_content(text)
            title, date, metadata = self.extract_metadata(soup)

            self.json_manager.insert_link(url, sanitized_text, title, date, metadata)
            logging.info(f"Successfully crawled: {url}")

            links = set()
            for a_tag in soup.find_all('a', href=True):
                link = a_tag['href']
                if link.startswith('http'):
                    links.add(link)

            return links
        except requests.RequestException as e:
            logging.error(f"Failed to crawl {url}: {e}")
            if retries_left > 0:
                logging.info(f"Retrying {url} ({retries_left} retries left)")
                time.sleep(5)
                return self.crawl(url, retries_left=retries_left - 1)
            else:
                logging.error(f"Failed to crawl after retries: {url}")
                return set()

    def run(self):
        while True:
            try:
                to_crawl = set(self.seed_urls)
                while to_crawl:
                    next_to_crawl = set()
                    for url in to_crawl:
                        if url not in [link['url'] for link in self.json_manager.get_all_links()]:
                            links = self.crawl(url)
                            next_to_crawl.update(links)
                    to_crawl = next_to_crawl
                time.sleep(10)
            except Exception as e:
                logging.error(f"LinkCrawler encountered an error: {e}")
                time.sleep(5)  # Wait before restarting

class ReverseIndexer:
    def __init__(self, json_manager):
        self.json_manager = json_manager
        self.indexing_complete = False

    def calculate_idf(self, term):
        try:
            doc_count = len(self.json_manager.get_all_links())
            term_docs = len(self.json_manager.get_reverse_index().get(term, {}))
            return math.log(doc_count / (1 + term_docs))
        except Exception as e:
            logging.error(f"Failed to calculate IDF for term {term}: {e}")
            return 0.0

    def calculate_tfidf(self, term, doc_id):
        try:
            tf = len(self.json_manager.get_reverse_index().get(term, {}).get(doc_id, []))
            idf = self.calculate_idf(term)
            return tf * idf
        except Exception as e:
            logging.error(f"Failed to calculate TF-IDF for term {term} in doc {doc_id}: {e}")
            return 0.0

    def rank_websites(self, query, top_n=10):
        try:
            results = []
            reverse_index = self.json_manager.get_reverse_index()

            for term in query.split():
                if term in reverse_index:
                    for doc_id, positions in reverse_index[term].items():
                        tfidf_score = self.calculate_tfidf(term, doc_id)
                        link_info = next((link for link in self.json_manager.get_all_links() if link['id'] == doc_id), None)
                        if link_info:
                            result = {
                                "site_name": link_info.get("title", ""),
                                "link_url": link_info["url"],
                                "score": tfidf_score,
                                "found_terms": term,
                                "frequency": len(positions)
                            }
                            results.append(result)

            results.sort(key=lambda x: x["score"], reverse=True)
            return results[:top_n]
        except Exception as e:
            logging.error(f"Failed to rank websites for query '{query}': {e}")
            return []

class QueryEngine:
    def __init__(self, reverse_indexer):
        self.reverse_indexer = reverse_indexer

    def query(self, search_terms):
        results = self.reverse_indexer.rank_websites(search_terms)
        if results:
            for result in results:
                print(f"Site Name: {result['site_name']}, URL: {result['link_url']}, Score: {result['score']}, Found Terms: {result['found_terms']}, Frequency: {result['frequency']}")
        else:
            print("No results found.")

class REPL:
    def __init__(self, query_engine):
        self.query_engine = query_engine

    def start(self):
        print("REPL started. Type 'exit' to quit.")
        while True:
            command = input(">>> ")
            if command.lower() == "exit":
                break
            elif command.lower().startswith("query"):
                query = command[6:]
                self.query_engine.query(query)
            else:
                print("Invalid command.")

if __name__ == "__main__":
    # Create and connect a TOR session
    tor_session = TorSession(tor_password='kiwi')
    tor_session.connect()

    # Initialize the JSON manager
    json_manager = JSONManager()

    # Seed URLs to start crawling
    seed_urls = [
        "http://torlinksge6enmcyyuxjpjkoouw4oorgdgeo7ftnq3zodj7g2zxi3kyd.onion/",
        "http://zqktlwiuavvvqqt4ybvgvi7tyo4hjl5xgfuvpdf6otjiycgwqbym2qad.onion/",
        "http://answerszuvs3gg2l64e6hmnryudl5zgrmwm3vh65hzszdghblddvfiqd.onion/",
        "http://libraryfyuybp7oyidyya3ah5xvwgyx6weauoini7zyz555litmmumad.onion",
        "http://nv3x2jozywh63fkohn5mwp2d73vasusjixn3im3ueof52fmbjsigw6ad.onion",
        "http://darkfailenbsdla5mal2mxn2uz66od5vtzd5qozslagrfzachha3f3id.onion",
        "http://dreadytofatroptsdj6io7l3xptbet6onoyno2yv7jicoxknyazubrad.onion ",
        "https://www.bbcnewsd73hkzno2ini43t4gblxvycyac5aw4gnv7t2rccijh7745uqd.onion",
        "http://http://p53lf57qovyuvwsc6xnrppyply3vtqm7l6pcobkmyqsiofyeznfu5uqd.onion",
        "https://www.nytimesn7cgmftshazwhfgzm37qxb44r64ytbb2dj3x62d2lljsciiyd.onion",
        "http://darkzzx4avcsuofgfez5zq75cqc4mprjvfqywo45dfcaxrwqg6qrlfid.onion/"

    ]

    # Initialize and start the link crawler
    crawler = LinkCrawler(tor_session, json_manager, seed_urls)
    crawler.start()

    # Initialize the reverse indexer
    indexer = ReverseIndexer(json_manager)

    # Initialize the query engine
    query_engine = QueryEngine(indexer)

    # Start REPL for database queries
    repl = REPL(query_engine)
    repl.start()

    # Ensure threads are joined before exiting
    crawler.join()

    logging.info("JSON-based search engine terminated.")
